...
async def get_page_data(session,sku_id):
    ...
    res = await start_request(session, url, headers=headers, proxy=proxy, return_type='text')
    # print(res)
    bs = BeautifulSoup(res, 'lxml')
    origin_price = bs.find('div', {'id': 'orginBuyBtn'}).span.get_text().strip()
    tuan_price = bs.find('div', {'id': 'tuanBtn'}).strong.get_text().strip()
    item_name = bs.find('div', {'id': 'itemName'}).get_text().strip()
    page_data = {'single_price': origin_price, 'tuan_price': tuan_price, 'item_name': item_name}
    return page_data
...
async def get_tuancount(session, sku_id):
    ...
    for i in range(2):
        try:
            proxy = await get_proxy(session)
            headers = await make_headers(session)
            res = await start_request(session, url, headers=headers, proxy=proxy)
                # res = await res.text()
                # res = json.loads(res)
    ...
    for i in range(2):
        try:
            ...
            # print(res)
                # res = await res.text()
                # res = json.loads(res)
            comment_count = res['result']['productCommentSummary']['CommentCount']
        except Exception as e:
            logger.exception(str(e))
        else:
