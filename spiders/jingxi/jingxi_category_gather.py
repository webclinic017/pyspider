...
async def make_headers(session):
    headers = {
        ...
        "user-agent": "Mozilla/5.0 (Linux; Android 8.0.0; Pixel 2 XL Build/OPD1.170816.004) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.83 Mobile Safari/537.36",
        "accept": "*/*",
    ...
    for i in range(3):
        try:
            async with session.get(
                url, headers=headers, proxy=proxy, timeout=3
            ) as resp:
                res = await resp.text()
            res = res[7:-2]
            data = json.loads(res)
