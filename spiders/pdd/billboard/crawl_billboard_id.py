...
def get_tab1_id():
    proxies = {
        "http": "http://2020061500002101216:cXr5v1Tm1MzF4RHK@forward.apeyun.com:9082"
    }
    url = "https://mobile.yangkeduo.com/pincard_share_card_popup.html"
    headers = {
        "user-agent": "Mozilla/5.0 (Linux; Android 8.1.0; PBBT00 Build/O11019; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/43.0.2357.65 Mobile Safari/537.36"
    }
...
async def get_tab2_id_set(session):
    ...
    await asyncio.gather(
        *[asyncio.create_task(get_tab2_id(tab_id, session)) for tab_id in tab1_set]
    )
